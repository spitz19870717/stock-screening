...
<header>
  <div class="container">
    <h1>📊 日本株 日次スクリーニングレポート</h1>
    <p class="header-meta">最終更新: {updated_at} ／ 対象: 日経225（{total_scanned}銘柄）</p>
  </div>
</header>
...
def generate(stocks, total_scanned=225):
    ...
    next_update = tomorrow.strftime("%Y年%m月%d日 12:00")

    glossary = _glossary_html()

    if not stocks:
        body = '<div class="no-results">今週は条件に合う銘柄がありませんでした。<br>来週のレポートをお待ちください。</div>'
